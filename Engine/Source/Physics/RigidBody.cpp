...
RigidBody::RigidBody(const Transform& transform, const Vector2& size, const def_t& def, const Physics& physics)
{
	...
	//bodyDef.rotation = b2MakeRot(transform.rotation); //switching to this from the above will give highly entertaining results
	...
	// set shape
	b2Vec2 hsize = ConvertVec2(Physics::ScreenToWorld(size * transform.scale * 0.5f)); //hsize is "half size"
	switch (def.shape)
	{
	case Shape::BOX:
	{
