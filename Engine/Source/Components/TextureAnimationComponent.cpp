...
void TextureAnimationComponent::Initialize()
{
	...
	//<call parent class (TextureComponent)Initialize()>
}
...
void TextureAnimationComponent::Update(float dt)
{
	...
	frameTimer -= dt;
		//<subtract delta time>
		if (frameTimer <= 0) //< frameTimer is less than or equal to 0
		{
			frameTimer = 1.0f / fps;
			frame += 1;
			//<increase frame by 1>
				if (frame> endFrame && loop) frame = startFrame; //<frame greater than endFrame and loop is true>
		}

	// calculate subimage size
	Vector2 size = texture->GetSize() / Vector2 {numColumns, numRows};
	//< texture size(GetSize()) / Vector2{ numColumns, numRows } >
	int column = (frame - 1) % numColumns;
	int row = (frame - 1) / numColumns;
	// set source rect from current column/row and subimage size
	source.x = (int)(column * size.x);  //<column * size x>
	source.y = (int)(row * size.y);  //<row * size y>
	source.w = (int)(size.x);  //<size x>
	source.h = (int)(size.y);   //<size y>
}
...
void TextureAnimationComponent::Read(const json_t& value)
{
	...
	//<Read all the Texture Animation Component data>
}
